...
#define	PROGRAM_VERSION             "05022016-141334"

/* config file */
#define CONFIG_FILE_PATH            "/etc/enstreamer/enstreamer.conf"
...
/*
 kill other services
 */
int minimum_service_mode(const int is_on) {
    return (is_on != 0) ? system("sudo systemctl stop encastrecv enslide ensynch") : -1;
}

/*
 loop video streaming
 */
int video_streaming_loop(const int is_on, const char *vidname) {
    if (is_on != 0) {
        if (strlen(vidname) == 0)
            return -1;
        ...
        /* update player info */
        write_play_status_to_file(vidname, "-", 1, "?", "?", 0);
...
int main(int argc, char** argv) {
    ...
    /* open config file */
    config_obj = iniparser_load(CONFIG_FILE_PATH);
    if (config_obj == NULL) {
        printf(KRED "Cannot parse file: %s\n" KRESET, CONFIG_FILE_PATH);
    } else {
        //        cmd_port = iniparser_getstring(config_obj, ":cmd_port", "1234");
        //kill other services than rtc
        minimum_service_mode(iniparser_getint(config_obj, ":limit_services", 0));

        //loop the video streaming all the way
        video_streaming_loop(iniparser_getint(config_obj, ":videostream_on", 0), iniparser_getstring(config_obj, ":videostream_url", ""));
    }

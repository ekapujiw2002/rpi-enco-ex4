...
#define	PROGRAM_VERSION             "26052015-182722"
...
void playlist_process_active(const int id_pls_active) {
    int id_active_items;

    /* play the playlist */
    for (id_active_items = playlist_controls[id_pls_active].id_item_start; id_active_items <= playlist_controls[id_pls_active].id_item_end; id_active_items++) {
...
int main(int argc, char** argv) {
    ...
    char flag_exit_to_main_loop = 0, flag_exit_to_next_playlist = 0, flag_playing_state = 0;
    int active_pls_id = -1, flag_audio_player_result = 0, playlist_next_id_items = 0;
...
ENSTREAMER_MAIN_LOOP:
    ...
    while (((active_pls_id = find_current_time_pls(playlist_total_item, playlist_controls)) != -1) && (flag_playlist_exit_result != 2)) {
        ...
        //                        if ((flag_playlist_exit_result == 3) || (flag_playlist_exit_result == 1) || ((flag_playlist_exit_result == 2) && (DATE_OLD != strToTime("00:00:00"))))
        //                            break;
    }
    ...
    for (;;) {
        ...
        if (isFileExists(current_pls)) {
            ...
            if (playlist_total_item != 0) {
                ...
                /* wait for start playing */
                flag_exit_to_main_loop = playlist_wait_to_start(&playlist_controls[0]);

                /* exit to main loop or play it ? */
                if (flag_exit_to_main_loop == 1) {
                    /* get active pls */
                    ...
                    if (active_pls_id != -1) {
                        /* play the playlist */
                        for (id_active_items = playlist_controls[active_pls_id].id_item_start; id_active_items <= playlist_controls[active_pls_id].id_item_end; id_active_items++) {
                            /* check item type */
                            /* audio */
                            if (playlist_items[id_active_items].item_type == ITEM_AUDIO) {
                                /* check id of playlist audio */
                                if (playlist_items[id_active_items].id_playlist_audio != -1) {
                                    printf(KYEL "Add audio playlist %d.m3u ... " RESET, playlist_items[id_active_items].id_playlist_audio);
                                    printf("%s\n" RESET, (flag_audio_player_result = mpc_load_playlist(playlist_items[id_active_items].id_playlist_audio, 0, 0)) == 1 ? KGRN "OK" : KRED "ERROR");

                                    /* wait all playlist item played */
                                    if (flag_audio_player_result == 1) {
                                        /* get status of mpc */
                                        flag_audio_player_result = mpc_get_info(&mpc_status_prev, playlist_items[id_active_items].is_online ? "00:00:00" : NULL);

                                        /* status == 0 ==> OK */
                                        if (flag_audio_player_result == 0) {
                                            /* set audio crossfaded */
                                            //                                            mpc_set_crossfade(playlist_items[id_active_items].crossfade);

                                            /* wait for all items played */
                                            for (;;) {
                                                /* get current status */
                                                //get mpc info
                                                flag_audio_player_result = mpc_get_info(&mpc_status_now, playlist_items[id_active_items].is_online ? "00:00:00" : NULL);

                                                /* process the info */
                                                if (flag_audio_player_result == 0) {

                                                    /* playing state update */
                                                    if ((strlen(mpc_status_now.song) == 0) && (mpc_status_now.status == 0) && (mpc_status_now.song_index == 0))
                                                        flag_playing_state = 1;
                                                    else if (mpc_status_now.song_index != mpc_status_prev.song_index)
                                                        flag_playing_state = 2;
                                                    else
                                                        flag_playing_state = 0;

                                                    /* display elapsed time */
                                                    //                                                    printf(KYEL "[%3d] %-60s\t%d\t%s" RESET, mpc_status_now.song_index, mpc_status_now.song, flag_playing_state, mpc_status_now.time_remaining);
                                                    printf(KYEL "%s" RESET, mpc_status_now.time_remaining);
                                                    fflush(stdout);
                                                    printf("\r");
                                                    fflush(stdout);

                                                    /* check for end of stream */
                                                    /* the last item played */
                                                    if (flag_playing_state == 1) {
                                                        printf(KRED "\nEnd of audio playlist (%d)\n" RESET, flag_playing_state);
                                                        //sleep(1);
                                                        //TODO : retest again to make sure

                                                        /* update prev status */
                                                        mpc_status_adjust_start_stop_time(&mpc_status_prev, 1, 1);

                                                        /* info playing previous */
                                                        //                                                        printf(KYEL "Status prev = %s %d %d %s %s %s %s %d\n" RESET, mpc_status_prev.song, mpc_status_prev.status, mpc_status_prev.song_index, mpc_status_prev.time_elapse, mpc_status_prev.time_total, mpc_status_prev.time_start, mpc_status_prev.time_stop, playlist_items[playlist_controls[id_active_items].id_item_start + mpc_status_prev.song_index].crossfade);
                                                        ...
                                                        /* just break */
                                                        printf(KGRN "Switch to next playlist\n" RESET);

                                                        /* update prev status */
                                                        //mpc_status_prev = mpc_status_now;

                                                        //check it is new date???
                                                        if (curr_date != strToTime("00:00:00")) {
                                                            flag_exit_to_main_loop = 1;
                                                            kill_audio_video_player();
                                                        }

                                                        /* update next id played */
                                                        playlist_next_id_items = id_active_items + mpc_status_prev.song_index + 1;
                                                        id_active_items = playlist_next_id_items;

                                                        break;
                                                    }/* still got another item to be play */
                                                    else if (flag_playing_state == 2) {
                                                        printf(KRED "\nEnd of audio playlist (%d)\n" RESET, flag_playing_state);
                                                        ...
                                                        //                                                                playlist_items[playlist_controls[id_active_items].id_item_start + mpc_status_prev.song_index].crossfade);

                                                        //CHECK FOR STATUS N=0 AND SONG_INDEX=0 AND SONG=EMPTY
                                                        //FOR END PLAYLIST
                                                        if ((strlen(mpc_status_now.song) == 0) && (mpc_status_now.status == 0)
                                                                /*
                                                                && (mpc_status_now.song_index==0)
                                                                 * 
                                                                 * */
                                                                ) {
                                                            printf(KRED "End of audio playlist\n" RESET);
                                                            //sleep(1);
                                                            //TODO : retest again to make sure
                                                            ...
                                                            //info playing previous
                                                            //                                                            printf(KYEL "Status prev = %s %d %d %s %s %s %s %d\n" RESET, mpc_status_prev.song, mpc_status_prev.status, mpc_status_prev.song_index, mpc_status_prev.time_elapse, mpc_status_prev.time_total, mpc_status_prev.time_start, playlist_items[playlist_controls[id_active_items].id_item_start + mpc_status_prev.song_index].crossfade);
                                                            ...
                                                            //info playing now
                                                            //                                                            printf(KYEL "Status now  = %s %d %d %s %s %s %s %d\n" RESET,
                                                            //                                                                    mpc_status_now.song,
                                                            ...
                                                            //                                                                    playlist_items[playlist_controls[id_active_items].id_item_start + mpc_status_prev.song_index].crossfade);
                                                            //set audio crossfade
                                                            //                                                            printf(KYEL "Set crossfade to %d ... %s\n" RESET,
                                                            //                                                                    playlist_items[playlist_controls[id_active_items].id_item_start + mpc_status_prev.song_index].crossfade,
                                                            //                                                                    (mpc_set_crossfade(playlist_items[playlist_controls[id_active_items].id_item_start + mpc_status_prev.song_index].crossfade)) == 0 ? KGRN "OK" : KRED "ERROR");
                                                    ...
                                                    printf(KRED "Error getting mpc status\n" RESET);
                                                }
                                            ...
                                            printf(KRED "MPC not loading!!! Will wait for 5secs...\n" RESET);
                                            sleep(5);
                                        }
                                    } else {
                                        printf(KRED "Cannot load audio playlist!!!\n" RESET);
                                    }
                                } else {
                                    printf(KRED "Invalid audio playlist id!!!\n" RESET);
                                }
                            }/* video */
                            else if (playlist_items[id_active_items].item_type == ITEM_VIDEO) {

                            }/* image */
                            else if (playlist_items[id_active_items].item_type == ITEM_IMAGE) {

                            }/* unknown item */
                            else {
                                printf(KRED "Unknown item type!!!\n" RESET);
                            }

                            /* continue to next playlist item */
                        }
                    } else {

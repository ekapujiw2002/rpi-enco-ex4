...
//program version
#define PROGRAM_INFO "BUZZER v25032017\r\n"
...
//server listen port
//char server_listen_port[32] = "udp://2222";
char server_listen_port[32] = "2222";

//server udp to connect to
char server_udp_client_url[64] = "192.168.1.15:2222";
char server_udp_client_host[64] = "192.168.1.15";
char server_udp_client_port[6] = "2222";

char is_realtime_activated = 0;
char is_local_beep_on = 0;
...
/**
 * send payload to udp client
 * @param payload
 */
void send_udp_packet(const char *payload) {
    if (sendto_inet_dgram_socket(socket_client_hdl, payload, strlen(payload) + 1, server_udp_client_host, server_udp_client_port, MSG_DONTWAIT) == -1) {
        log_data("Command queued (%d)", errno);
    } else {
        log_data("Command sent : %s", payload);
    }
}
...
int main(int argc, char** argv) {
    ...
    // scheduling priority
    sched_param paramx = {sched_get_priority_max(SCHED_RR)};

    /*
     parse option
     */
    while ((option = getopt(argc, argv, "c:f:s:Rl")) != -1) {
        switch (option) {
            case 'c':
    ...
    if (is_realtime_activated) {
        log_data("Set priority to real time with priority %d : %s (%d)", paramx.__sched_priority, (sched_setscheduler(0, SCHED_RR, &paramx) == 0) ? "OK" : "ERROR", errno);
    } else {

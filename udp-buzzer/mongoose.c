...
void mg_tun_send_frame(struct mg_connection *ws, uint32_t stream_id,
    ...
    stream_id = htonl(stream_id);
    {
        struct mg_str parts[] = {
            ...
            {msg.p, msg.len}
            /* vc6 doesn't like just `msg` here */
        };
...
struct mg_connection *mg_sntp_connect(struct mg_mgr *server_conn_mgr,
        mg_event_handler_t event_handler,
    ...
    c = mg_connect(server_conn_mgr, p_url, event_handler);
...
struct mg_connection *mg_sntp_get_time(struct mg_mgr *server_conn_mgr,
        mg_event_handler_t event_handler,
        const char *sntp_server_name) {
    struct mg_connection *c;
    ...
    c = mg_sntp_connect(server_conn_mgr, mg_sntp_util_ev_handler, sntp_server_name);
    if (c == NULL) {
        MG_FREE(sd);
        return NULL;
    }
...
struct mg_q_msg {
    ...
    void (*cb)(struct mg_mgr *server_conn_mgr, void *arg);
    void *arg;
};
...
static void mg_task(void *arg) {
    ...
    mg_init(&server_conn_mgr);
    while (1) {
        struct mg_q_msg msg;
        mg_mgr_poll(&server_conn_mgr, 1);
        if (osi_MsgQRead(&s_mg_q, &msg, 1) != OSI_OK) continue;
        switch (msg.type) {
            case MG_Q_MSG_CB:
            {
                msg.cb(&server_conn_mgr, msg.arg);
            }
        }
    }
}

void mg_run_in_task(void (*cb)(struct mg_mgr *server_conn_mgr, void *arg), void *cb_arg) {
...
time_t mg_sl_if_poll(struct mg_iface *iface, int timeout_ms) {
    struct mg_mgr *server_conn_mgr = iface->mgr;
    double now = mg_time();
    double min_timer;
    struct mg_connection *nc, *tmp;
     ...
     */
    min_timer = 0;
    for (nc = server_conn_mgr->active_connections, num_fds = 0; nc != NULL; nc = tmp) {
        tmp = nc->next;
    ...
    for (nc = server_conn_mgr->active_connections; nc != NULL; nc = tmp) {
        int fd_flags = 0;
        if (nc->sock != INVALID_SOCKET) {
            if (num_ev > 0) {
                fd_flags =
...
void sl_restart_cb(struct mg_mgr *server_conn_mgr) {
    /*
     ...
     */
    struct mg_connection *nc;
    for (nc = mg_next(server_conn_mgr, NULL); nc != NULL; nc = mg_next(server_conn_mgr, nc)) {
...
void mg_lwip_mgr_schedule_poll(struct mg_mgr *server_conn_mgr);
...
void mg_ev_mgr_lwip_process_signals(struct mg_mgr *server_conn_mgr) {
    struct mg_ev_mgr_lwip_data *md =
            (struct mg_ev_mgr_lwip_data *) server_conn_mgr->ifaces[MG_MAIN_IFACE]->data;
...
#endif
    mg_ev_mgr_lwip_process_signals(server_conn_mgr);
    for (nc = server_conn_mgr->active_connections; nc != NULL; nc = tmp) {
        struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
        tmp = nc->next;
        n++;
...
uint32_t mg_lwip_get_poll_delay_ms(struct mg_mgr *server_conn_mgr) {
    struct mg_connection *nc;
    double now = mg_time();
    double min_timer = 0;
    int num_timers = 0;
